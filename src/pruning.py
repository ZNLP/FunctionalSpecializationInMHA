...
def what_to_prune(
    ...
    # prune the most important heads first 
    heads_and_score = sorted(heads_and_score, key=lambda x: x[1], reverse=not ascending)
    # prune the less important heads first
    # heads_and_score = sorted(heads_and_score, key=lambda x: x[1])
    
    logger.info(f"++++Heads and their scores(Ascending Order:{ascending}):")
    for i in heads_and_score:
        logger.info(f"Layer-{i[0][0]},Head-{i[0][1]},Score-{i[1]}")
